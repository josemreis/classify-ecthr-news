...
tf_main <- recipe(ecthr_label ~ ., 
  ...
  step_tokenfilter(text, min_times = 2)  %>%
  step_tf(text)
## the recipe
tf_main
```
...
set.seed(1234)
ecthr_boot <- bootstraps(train_data, times = 10, strata = "ecthr_label")
ecthr_boot
```
...
best_penalty_tf <- select_best(lasso_grid_tf, metric =  "f_meas")
show_best(lasso_grid_tf, metric =  "f_meas")
best_penalty_tfidf <- select_best(lasso_grid_tfidf, metric =  "f_meas")
show_best(lasso_grid_tfidf, metric =  "f_meas")
