...
**add the glove embeddings description!**
...
5-fold cross validation with three repeats (stratified by the outcome variable) from the training data for cross-validation during tuning of the training models.

```{r}
# 10-fold cross-validation
set.seed(1234)
cv_samples <- vfold_cv(train_data, v = 5, repeats = 3, strata = "ecthr_label")
cv_samples
```
...
Pre-processing recipe:
  ...
  - pre-trained glove embeddings (50 dim) for feature reduction; more details above, also [see](http://www.aclweb.org/anthology/D14-1162)
...
embeddings <- textdata::embedding_glove6b(dimensions = 50)
...
```{r}
if (!file.exists("models/glove-lasso_model.Rds")) {
  ## parallel
  ...
  (p1 <- autoplot(lasso_bayes_tune) +
      theme_minimal() + 
      ggtitle("glove-lasso model"))
  
  ggsave(p1, 
         filename = "models/metrics/plots/glove-lasso_marginals.png",
         width = 8,
         height = 6,
         device = "png")
  ...
  ggsave(p2, 
         filename = "models/metrics/plots/glove-lasso_parameters.png",
         width = 8,
         height = 6,
         device = "png")
  ...
  ggsave(p3, 
         filename = "models/metrics/plots/glove-lasso_performance.png",
         width = 8,
         height = 6,
         device = "png")
  ...
  lasso_bayes_tune %>%
    collect_metrics() %>%
    mutate(model = "lasso-glove") %>%
    write_csv(path = "models/metrics/glove-lasso_metrics_tune.csv")

} else {
  
  readr::read_csv("models/metrics/glove-lasso_metrics_tune.csv") %>%
...
if (!file.exists("models/glove-lasso_model.Rds")) {
  ...
  readr::read_rds("models/glove-lasso_workflow.Rds") %>%
    pluck(".workflow", 1)
...
if (!file.exists("models/glove-lasso_model.Rds")) {
  ...
  test_performance %>%
    mutate(model = "lasso-glove") %>%
    write_csv(path = "models/metrics/glove-lasso_metrics.csv")
  
} else {
  
  readr::read_csv("models/metrics/glove-lasso_metrics.csv")
...
# } else {
#   
#   (p <- readr::read_rds("models/glove-lasso_model.Rds") %>%  
#   pull_workflow_fit() %>% 
#   vip::vip(num_features = 40) + 
#   theme_minimal())
# 
...
if (!file.exists("models/glove-lasso_model.Rds")) {
  ...
  write_rds(final_model,
            path =  "models/glove-lasso_model.Rds",
            compress = "gz")
  
  write_rds(lasso_fit,
            path =  "models/glove-lasso_workflow.Rds",
            compress = "gz")
  
  lasso_fit %>% 
    pluck(".workflow", 1) %>%
    pull_workflow_prepped_recipe() %>%
    write_rds(path =  "models/glove-lasso_prep_recipe.Rds",
            compress = "gz")
...
if (!file.exists("models/rf-glove_model.Rds")) {
  
  ## set up the parameters efore hand
  rf_param <- rf_cs_wf %>%
    parameters() %>%
    update(mtry = finalize(mtry(), select(juice(prep(glove_main)), -ecthr_label)))
  ...
  (p1 <- autoplot(rf_bayes_tune) +
      theme_minimal() + 
      ggtitle("glove-rf model"))
  
  ggsave(p1, 
         filename = "models/metrics/plots/rf-glove_marginals.png",
         width = 8,
         height = 6,
         device = "png")
  ...
  ggsave(p2, 
         filename = "models/metrics/plots/rf-glove_parameters.png",
         width = 8,
         height = 6,
         device = "png")
  ...
  ggsave(p3, 
         filename = "models/metrics/plots/rf-glove_performance.png",
         width = 8,
         height = 6,
         device = "png")
  ...
  rf_bayes_tune %>%
    collect_metrics() %>%
    mutate(model = "rf-glove") %>%
    write_csv(path = "models/metrics/rf-glove_metrics_tune.csv")

} else {
  
  ## logging because numbers are almost undistinguishable otherwise
  readr::read_csv("models/metrics/rf-glove_metrics_tune.csv") %>%
...
if (!file.exists("models/rf-glove_model.Rds")) {
  ...
  readr::read_rds("models/rf-glove_workflow.Rds") %>% 
  pluck(".workflow", 1)
...
if (!file.exists("models/rf-glove_model.Rds")) {
  ...
  test_performance %>%
    mutate(model = "rf-glove") %>%
    write_csv(path = "models/metrics/rf-glove_metrics.csv")

} else {
  
  readr::read_csv("models/metrics/rf-glove_metrics.csv")
...
# } else {
#   
#   (p <- readr::read_rds("models/rf-glove_model.Rds") %>%  
#   pull_workflow_fit() %>% 
#   vip::vip(num_features = 40) + 
#   theme_minimal())
# 
...
if (!file.exists("models/rf-glove_model.Rds")) {
  ...
  write_rds(final_model,
            path =  "models/rf-glove_model.Rds",
            compress = "gz")
  
  write_rds(rf_fit,
            path =  "models/rf-glove_workflow.Rds",
            compress = "gz")
...
if (!file.exists("models/svmrbf-glove_model.Rds")) {
  ...
  ggsave(p3, 
         filename = "models/metrics/plots/svmrbf-glove_performance.png",
         width = 8,
         height = 6,
         device = "png")
...
if (!file.exists("models/svmrbf-glove_model.Rds")) {
  ...
  readr::read_rds("models/svmrbf-glove_workflow.Rds") %>% 
  pluck(".workflow", 1)
...
if (!file.exists("models/svmrbf-glove_model.Rds")) {
  ...
  test_performance %>%
    mutate(model = "svm-rbf-glove") %>%
    write_csv(path = "models/metrics/svmrbf-glove_metrics.csv")

} else {
  
  readr::read_csv("models/metrics/svmrbf-glove_metrics.csv")
...
if (!file.exists("models/svmrbf-glove_model.Rds")) {
  ...
  write_rds(final_model,
            path =  "models/svmrbf-glove_model.Rds",
            compress = "gz")
  
  write_rds(svmrbf_fit,
            path =  "models/svmrbf-glove_workflow.Rds",
            compress = "gz")
  
  svmrbf_fit %>% 
    pluck(".workflow", 1) %>%
    pull_workflow_prepped_recipe() %>%
    write_rds(path =  "models/svmrbf-glove_prep_recipe.Rds",
            compress = "gz")
...
if (!file.exists("models/polysvm-glove_model.Rds")) {
  
  doParallel::registerDoParallel()
  set.seed(1234)
  poly_svm_bayes_tune <- tune_bayes(
    poly_svm_wf,
    resamples = cv_samples,
    param_info = parameters(
      cost(),
  ...
  ## plot the metrics
  (p1 <- autoplot(poly_svm_bayes_tune) +
      theme_minimal() + 
      ggtitle("glove-poly-svm model"))
  
  ggsave(p1, 
         filename = "models/metrics/plots/polysvm_marginals.png",
         width = 8,
         height = 6,
         device = "png")
  
  (p2 <- autoplot(poly_svm_bayes_tune, type = "parameters") +
      theme_minimal() + 
      ggtitle("glove-poly-svm model"))
  
  ggsave(p2, 
         filename = "models/metrics/plots/polysvm_parameters.png",
         width = 8,
         height = 6,
         device = "png")
  
  (p3 <- autoplot(poly_svm_bayes_tune, type = "performance") +
      theme_minimal() + 
      ggtitle("glove-poly-svm model"))
  
  ggsave(p3, 
         filename = "models/metrics/plots/polysvm-glove_performance.png",
         width = 8,
         height = 6,
         device = "png")
  ...
  poly_svm_bayes_tune %>%
    collect_metrics() %>%
    mutate(model = "poly-svm-glove") %>%
    write_csv(path = "models/metrics/polysvm_metrics_tune.csv")

} else {
  
  ## logging because numbers are almost undistinguishable otherwise
  readr::read_csv("models/metrics/polysvm_metrics_tune.csv") %>%
    ggplot(aes(x = mtry, y = mean, color = factor(.metric))) + 
    geom_point() + 
    geom_line() +
    theme_minimal() + 
...
if (!file.exists("models/polysvm-glove_model.Rds")) {
  ...
  # select best model by cohen's kappa
  final_model <- poly_svm_bayes_tune %>%
    select_best(metric = "kap")
  
  ## finalize
  final_polysvm_wf <- finalize_workflow(
    poly_svm_wf,
    final_model
  )
  ...
  readr::read_rds("models/polysvm_workflow.Rds") %>% 
  pluck(".workflow", 1)
...
if (!file.exists("models/polysvm-glove_model.Rds")) {
  ...
  test_performance %>%
    mutate(model = "poly-svm-glove") %>%
    write_csv(path = "models/metrics/polysvm-glove_metrics.csv")

} else {
  
  readr::read_csv("models/metrics/polysvm-glove_metrics.csv")
...
if (!file.exists("models/polysvm-glove_model.Rds")) {
  ...
  readr::read_rds("models/polysvm-glove_workflow.Rds") %>% 
    collect_predictions() %>%
    conf_mat(ecthr_label, .pred_class)
...
if (!file.exists("models/polysvm-glove_model.Rds")) {
  ...
  write_rds(final_model,
            path =  "models/polysvm-glove_model.Rds",
            compress = "gz")
  
  write_rds(svmrbf_fit,
            path =  "models/polysvm-glove_workflow.Rds",
            compress = "gz")
  
  svmrbf_fit %>% 
    pluck(".workflow", 1) %>%
    pull_workflow_prepped_recipe() %>%
    write_rds(path =  "models/polysvm-glove_prep_recipe.Rds",
            compress = "gz")
...
if (!file.exists("models/xgb-glove_model.Rds")) {
  ...
  (p1 <- autoplot(xgb_bayes_tune) +
      theme_minimal() + 
      ggtitle("glove-xgb model"))
  
  ggsave(p1, 
         filename = "models/metrics/plots/xgb_marginals.png",
         width = 8,
         height = 6,
         device = "png")
  ...
  ggsave(p3, 
         filename = "models/metrics/plots/xgb-glove_performance.png",
         width = 8,
         height = 6,
         device = "png")
...
if (!file.exists("models/xgb-glove_model.Rds")) {
  ...
  readr::read_rds("models/xgb-glove_workflow.Rds") %>% 
  pluck(".workflow", 1)
...
if (!file.exists("models/xgb-glove_model.Rds")) {
  ...
  # export
  test_performance %>%
    mutate(model = "xgb-glove") %>%
    write_csv(path = "models/metrics/xgb-glove_metrics.csv")

} else {
  
  readr::read_csv("models/metrics/xgb-glove_metrics.csv")
...
if (!file.exists("models/xgb-glove_model.Rds")) {
  ...
  write_rds(final_model,
            path =  "models/xgb-glove_model.Rds",
            compress = "gz")
  
  write_rds(xgb_fit,
            path =  "models/xgb-glove_workflow.Rds",
            compress = "gz")
  
  xgb_fit %>% 
    pluck(".workflow", 1) %>%
    pull_workflow_prepped_recipe() %>%
    write_rds(path =  "models/xgb-glove_prep_recipe.Rds",
            compress = "gz")
