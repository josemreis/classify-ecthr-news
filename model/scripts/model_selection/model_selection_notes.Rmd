...
tfidf_main <- recipe(ecthr_label ~ ., 
  ...
  step_tokenfilter(text, min_times = 2, max_tokens = tune()) %>%
...
Tunning the model with Bayesian model-based optimization [see](https://towardsdatascience.com/a-conceptual-explanation-of-bayesian-model-based-hyperparameter-optimization-for-machine-learning-b8172278050f). Tunning

  - Penalty term (lambda) - default values from ```glmnet```;
  - Max tokens (top-n tokens to keep) - range between 300 and 1000 tokens;

```{r}
lasso_bayes_tune <- tune_bayes(
  lasso_wf,
  resamples = cv_samples,
  # Going with default range values for lambda, but specific range for max tokens
  param_info = param_set(penalty(), max_tokens(range(300, 1000))),
  # Generate five at semi-random to start
  initial = 5,
  iter = 50,
  # How to measure performance?
  metrics = metric_set(roc_auc, bal_accuracy, f_meas, ppv, npv, recall, precision, kap),
  control = control_bayes(no_improve = 20, verbose = FALSE, save_pred = TRUE)
)
  
## plot the metrics
autoplot(lasso_bayes_tune) +
theme_minimal() + 
ggtitle("tfidf-lasso model")

autoplot(lasso_bayes_tune, type = "parameters") +
theme_minimal() + 
ggtitle("tfidf-lasso model")

autoplot(lasso_bayes_tune, type = "performance") +
theme_minimal() + 
ggtitle("tfidf-lasso model")

### Confusion matrices by f_meas
lasso_bayes_tune %>%
collect_predictions() %>%
filter(
  .config == select_best(
    lasso_bayes_tune, metric = "lasso_bayes_tune"
    ) %>%
    pull(.config)
  ) %>%
...
lasso_wf_tfidf,
final_model %>% select(penalty, max_tokens, .config)
)
...
Next, a random forests models following ``ranger``'s implementation. Will implement a cost-sensitive one (i.e. highest cost to miss-classifications of the minority class)
...
if (exists("tfidf_main")) {
  ...
  final_workflow <- recipe(ecthr_label ~ ., data = train_data) %>%
  update_role(model_id, new_role = "id variable") %>%
  step_dummy(lang_og) %>%
