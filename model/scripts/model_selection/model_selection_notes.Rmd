...
Pre-processing recipe:
  ...
  - word vectors fasttext/glove (respectively 50 dim and pre-trained; 100 dim and trained on corpus) for feature reduction; more details above, also [see](http://www.aclweb.org/anthology/D14-1162) [as well as](https://arxiv.org/abs/1607.04606)
...
if (!file.exists("models/rf-wv_model.Rds")) {
  ...
  rf_param <- rf_cs_wf %>%
    parameters() %>%
    update(mtry = finalize(mtry(), select(juice(prep(wv_main)), -ecthr_label)))
...
if (!file.exists("models/polysvm-wv_model.Rds")) {
  ...
  ggsave(p3, 
         filename = "models/metrics/plots/polysvm-wv_performance.png",
         width = 8,
         height = 6,
         device = "png")
...
if (!file.exists("models/polysvm-wv_model.Rds")) {
  ...
  test_performance %>%
    mutate(model = "poly-svm-wv") %>%
    write_csv(path = "models/metrics/polysvm-wv_metrics.csv")

} else {
  
  readr::read_csv("models/metrics/polysvm-wv_metrics.csv")
...
if (!file.exists("models/polysvm-wv_model.Rds")) {
  ...
  readr::read_rds("models/polysvm-wv_workflow.Rds") %>% 
    collect_predictions() %>%
    conf_mat(ecthr_label, .pred_class)
...
if (!file.exists("models/polysvm-wv_model.Rds")) {
  ...
  write_rds(final_model,
            path =  "models/polysvm-wv_model.Rds",
            compress = "gz")
  
  write_rds(svmrbf_fit,
            path =  "models/polysvm-wv_workflow.Rds",
            compress = "gz")
  
  svmrbf_fit %>% 
    pluck(".workflow", 1) %>%
    pull_workflow_prepped_recipe() %>%
    write_rds(path =  "models/polysvm-wv_prep_recipe.Rds",
            compress = "gz")
