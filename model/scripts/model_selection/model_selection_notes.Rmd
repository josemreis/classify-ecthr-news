...
glove_main <- recipe(
  ...
  step_smote(ecthr_label, neighbors = 30) ## smote sampling, k = 30
...
if (!file.exists("models/glove-lasso_model.Rds")) {
  ...
  lasso_bayes_tune <- tune_bayes(
    ...
    metrics = metric_set(kap, roc_auc, bal_accuracy, f_meas, ppv, npv, recall, precision),
...
if (!file.exists("models/rf-glove_model.Rds")) {
  ...
  rf_bayes_tune <- tune_bayes(
    ...
    param_info = parameters(
      mtry() %>% range_set(c(8, 60))
      ),
    # Generate five at semi-random to start
    initial = 25,
    iter = 50,
    # How to measure performance? Select bbest based on cohens kappa
    metrics = metric_set(kap, roc_auc, bal_accuracy, f_meas, ppv, npv, recall, precision),
    control = control_bayes(no_improve = 15, verbose = FALSE, save_pred = TRUE, seed = 1234)
)
